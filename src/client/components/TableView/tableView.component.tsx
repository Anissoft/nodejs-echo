...
export const TableView = memo(<T extends Record<string, any>>({
  items,
  columns,
  className,
  selectedId,
  ...
  const head: ItemHead<T>[] = useMemo(() => columns.map(column => {
    const sortable = column.sortable ?? true;
    const isSorted = (sortable && sorting && sorting[0] === column.key);
    ...
    const compare = column.compare ?? ((a, b) => 
      (getValue(a) > getValue(b) ? -1 : 1) * sortDirection
    );
    const onClick = () => {
      if (!sortable) {
        return;
      }
      if (!sorting || sorting[0] !== column.key) {
